            ...
            <div className="bg-slate-900/60 p-6 rounded-2xl border border-slate-800">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Quét kịch bản từ Phần 1 để tự động thiết lập hồ sơ tạo hình nhất quán (tiếng Anh chuyên sâu &gt;250 từ kèm khối Donghua Art Style).
                  </p>
                </div>

                {scenes.length === 0 ? (
                  <div className="text-xs bg-amber-500/10 text-amber-300 px-3 py-1.5 rounded-lg border border-amber-500/20">
                    Bạn cần tách kịch bản ở Phần 1 trước.
                  </div>
                ) : (
                  <button
                    ...
                    className="px-4 py-2 bg-gradient-to-r from-rose-500 to-amber-500 text-slate-950 font-bold rounded-xl text-xs transition-all shadow-md flex items-center gap-1.5 disabled:opacity-50"
                  >
                    {isGeneratingDNA ? (
                      <>
              ...
              {dnaProfiles.length === 0 && !isGeneratingDNA && (
                <div className="border border-dashed border-slate-800 rounded-xl p-8 text-center text-slate-500">
                  ...
                  <p className="text-xs mt-1">
                    Nhấn nút <span className="text-rose-400 font-bold">"Khởi Tạo Hệ Thống ADN"</span> phía trên để AI tiến hành rà soát các thực thể trong truyện.
                  </p>
                </div>
              )}
